...
describe "Editing todo lists" do
  let!(:todo_list) {  TodoList.create(title: "Complete", description: "Completes an item successfully.")}
  let!(:todo_item) {  todo_list.todo_items.create(content: "To be Completed")}
...
context "with completed items" do
  let!(:completed_todo_item) {  todo_list.todo_items.create(content: "To be Completed", completed_at: 5.minutes.ago)}
  ...
  it "does not give the option to mark completed items as complete" do
    visit_todo_list todo_list
    within dom_id_for(completed_todo_item) do
